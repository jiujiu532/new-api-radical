...
// InvitationCode 邀请码/注册码模型
type InvitationCode struct {
	Id        int    `json:"id" gorm:"primaryKey;autoIncrement"`
	Code      string `json:"code" gorm:"type:varchar(64);uniqueIndex;not null"`       // 邀请码
...
// InvitationCodeUsageLog 邀请码使用记录
type InvitationCodeUsageLog struct {
	Id        int    `json:"id" gorm:"primaryKey;autoIncrement"`
	CodeId    int    `json:"code_id" gorm:"index;not null"`                           // 邀请码ID
	Code      string `json:"code" gorm:"type:varchar(64);index"`                      // 邀请码
...
func ValidateAndUseCode(code string, codeType int, userId int, username string, ip string) (*InvitationCode, error) {
	...
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, errors.New("邀请码不存在或类型不匹配")
		}
		return nil, err
	}
...
// GetInvitationCodeById 根据ID获取邀请码
...
// CheckCodeExists 检查邀请码是否存在且有效（不消耗使用次数）
