...
const CODE_STATUS = {
    ...
    3: { text: '已禁用', color: 'red', icon: <IconClose /> },
};
...
const InvitationCode = () => {
    ...
    const handleGenerate = async () => {
        try {
            const payload = {
                ...generateForm,
                expired_at: generateForm.expired_at ? generateForm.expired_at.unix() : 0,
            };
    ...
    // 按类型删除所有
    const handleDeleteAllByType = async (codeType) => {
        try {
            const res = await API.delete(`/api/invitation_code/delete_all_by_type?type=${codeType}`);
            if (res.data.success) {
    ...
    const columns = [
        ...
        },
        {
            ...
            width: 100,
            render: (status) => (
                <Tag color={CODE_STATUS[status]?.color}>
                    {CODE_STATUS[status]?.icon} {CODE_STATUS[status]?.text || '未知'}
                </Tag>
            ),
        },
        {
            ...
            render: (used, record) => {
                ...
                const percent = record.max_uses === -1 ? 0 : (used / record.max_uses) * 100;
                return (
        ...
        <div style={{
            ...
            <Card style={{ marginBottom: 12, flexShrink: 0 }} bodyStyle={{ padding: '12px' }}>
                <Space>
                    ...
                    <Popconfirm
                        title="确定删除所有注册码？此操作不可撤销！"
                        onConfirm={() => handleDeleteAllByType(1)}
                    >
                        <Button type="danger">删除所有注册码</Button>
                    </Popconfirm>
                    <Popconfirm
                        title="确定删除所有解封码？此操作不可撤销！"
                        onConfirm={() => handleDeleteAllByType(2)}
                    >
                        <Button type="danger">删除所有解封码</Button>
                    </Popconfirm>
                </Space>
            </Card>
